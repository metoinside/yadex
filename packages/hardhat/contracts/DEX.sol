...
contract DEX {
	...
	function init(uint256 tokens) public payable returns (uint256) {
		...
		require(msg.value == tokens, "Token pairs are not enough");
	...
	function price(
		...
		//Formula is (ydx / x + dx) where in dx needs to be multiplied by 0.997 for the trading fee
		yOutput =
	...
	function ethToToken() public payable returns (uint256 tokenOutput) {
		require(msg.value > 0, "0 value is not permitted");
		tokenOutput = price(
			msg.value,
		...
		require(
			token.transfer(address(msg.sender), tokenOutput),
			"Unable to transfer"
		);
	...
	function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
		require(tokenInput > 0, "0 value is not permitted");
		ethOutput = price(
			tokenInput,
			token.balanceOf(address(this)),
		...
		require(
			...
			"Unable to transfer, ensure you approved first"
		);
	...
	function deposit() public payable returns (uint256 tokensDeposited) {
		require(msg.value > 0, "0 is not allowed");

		uint256 tokenEthRatio = token.balanceOf(address(this)) /
			(address(this).balance - msg.value);

		//ETH is auto accepted as it is a payable function, let's set $BAL
		tokensDeposited = msg.value * tokenEthRatio;
		require(
	...
	function withdraw(
		...
		uint256 tokenEthRatio = token.balanceOf(address(this)) /
			(address(this).balance);
		eth_amount = amount * tokenEthRatio;

		(bool sent, ) = payable(msg.sender).call{ value: eth_amount }("");
		require(sent, "Unable to withdraw");
		require(token.transfer(msg.sender, amount), "Unable to withdraw");
